...
## Program to load AMX files from Loggerhead AMX
  ...
  while (notdone)
    ...
    for i = 1:12
      SID_SPEC(nSIDSPEC).sensor.name = [SID_SPEC(nSIDSPEC).sensor.name;
					fread(fid, STR_MAX, 'uint8=>char')'];
    end
    for i = 1:12
      SID_SPEC(nSIDSPEC).sensor.units = [SID_SPEC(nSIDSPEC).sensor.units;
					 fread(fid, STR_MAX, 'uint8=>char')'];
    end
    for i = 1:12
      SID_SPEC(nSIDSPEC).sensor.cal = [SID_SPEC(nSIDSPEC).sensor.cal;
				       fread(fid, 1, 'float32')'];
    end
  ...
  while (eofstat == 0)
	...
	if ((SID_SPEC(cur_sid).SID(1) == 'I') ||
	    (SID_SPEC(cur_sid).SID(1) == '3'))
          SID_REC(reccounter).data = fread(fid, SID_SPEC(cur_sid).nSamples,
					   'int16', 'ieee-be');
	else
          SID_REC(reccounter).data = fread(fid, SID_SPEC(cur_sid).nSamples,
					   'int16');
